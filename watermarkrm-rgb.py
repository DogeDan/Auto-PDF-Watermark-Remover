...
def extract_pdf_to_images(pdf_path, output_folder, dpi=300, debug=False):
    ...
    if debug:
        ...
        print(f"\n{'='*50}")
        print(f"Step 1: Render PDF pages to images")
        print(f"{'='*50}")
    ...
    for page_num in range(total_pages):
        ...
        try:
            ...
            try:
                ...
                # Fallback: use lower DPI or different rendering options
                try:
            ...
            if debug:
                # Save as PNG when in debug mode
                ...
                # Append image array to result list
                image_data.append(img_array)

        except Exception as e:
            error_msg = f"Error: Unable to process page {page_num + 1}: {e}"
            if debug:
                print(f"\n{error_msg}")
            else:
                print(error_msg)
            # Create a blank placeholder page and append
...
def remove_watermark_from_image(image_input, watermark_color_rgb, output_path=None):
    """
    Step 2: Remove watermark from a single image using a provided RGB color center.

    Args:
        image_input: Path to an image file or a numpy image array.
        watermark_color_rgb: Watermark RGB color tuple used as the mask center.
    ...
    # Build a BGR range mask around the input color (per-channel tolerance)
    tol = 25  # per-channel tolerance
...
def process_images_in_folder(input_data, output_folder, watermark_color_rgb, debug=False):
    """
    ...
    Args:
        ...
        watermark_color_rgb: Watermark color as (R, G, B)
    ...
    if debug:
        ...
        print(f"Output folder: {output_path.absolute()}")
        print(f"Watermark color: RGB({watermark_color_rgb[0]}, {watermark_color_rgb[1]}, {watermark_color_rgb[2]})\n")
        ...
        for idx, image_file in enumerate(image_files, 1):
            ...
            remove_watermark_from_image(image_file, watermark_color_rgb, output_file)
        ...
        for img_array in input_data:
            processed_img = remove_watermark_from_image(img_array, watermark_color_rgb)
...
All PDF files in the current directory will be processed and output to ./output/
    ...
    watermark_color = (args.red, args.green, args.blue)
    ...
    for pdf_file in pdf_files:
        ...
        try:
            if args.debug:
                ...
                processed_paths = process_images_in_folder(
                    images_extracted_folder,
                    images_cleaned_folder,
                    watermark_color,
                    debug=True
                )
                ...
                # Step 2: Process all images
                processed_arrays = process_images_in_folder(
                    image_arrays,
                    None,
                    watermark_color,
                    debug=False
